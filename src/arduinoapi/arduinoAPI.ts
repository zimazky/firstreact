export default class ArduinoController {
  ...
  getInfo(response = (t: string)=>{}, reject = (e: any)=>{}) {
  ...
  setTemperature(zone: number, temperature: number, response = ()=>{}, reject = ()=>{}) {
  ...
  powerOn(zone: number, response = ()=>{}, reject = ()=>{}) {
  ...
  powerOff(zone: number, response = ()=>{}, reject = ()=>{}) {
  ...
  static parseInfo(text: string) {
    ...
    function parseZonesInfo(i: string[], a = [], id = 0) {
  ...
  static sensorState = (s: number) => ArduinoController.states[-s]
}	
